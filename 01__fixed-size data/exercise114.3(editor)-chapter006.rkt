...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname |exercise114.3(editor)-chapter006|) (read-case-sensitive #t) (teachpacks ((lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp")) #f)))
; exercise114.3(editor)-chapter006

; Q.:
; Use the predicates from exercise 113 to check the editor program (A Graphical Editor).
...
(define (run ed)
  (big-bang ed)
    [check-with editor?] ; exercise114.3(editor)-chapter006
    [to-draw render]
    [on-key edit])

;(run 1) ; check-with: the initial expression evaluated to 1, which fails to pass check-with's editor? test
;(run (make-editor "hello" "world")) ; successful program execution
