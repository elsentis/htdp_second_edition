...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname exercise106-chapter006) (read-case-sensitive #t) (teachpacks ((lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "universe.rkt" "teachpack" "2htdp") (lib "image.rkt" "teachpack" "2htdp")) #f)))
; exercise106-chapter006
...
(define H-GAUGE-BACKGROUND (empty-scene WIDTH ANIMAL-GAUGE-HEIGHT))
...
(define ANIMAL-X-INCREASE-PER-TOCK (/ WIDTH 300))
...
(define (aux-render-animal-image-vcat x)
  (cond [(even? (remainder (* x 10) 10)) cat1]
        [ else cat2]))
...
;(check-expect (tock CAT1) (make-vcat (+ 50 INCREASE_X_PER_TOCK) (- 50 DECREASE_H_PER_TOCK) "right"))
;(check-expect (aux-render-animal-image-vcat 31.6) IMAGE-CAT1)
;(check-expect (aux-render-animal-image-vcat 31) IMAGE-CAT1)

(define (tock animal)
  (cond [(vcat? animal) (make-vcat
                         (aux-tock-x-cat (vcat-x animal) (vcat-dir animal))
                         (aux-tock-hap (vcat-hap animal))
                         (aux-tock-dir (vcat-x animal) (aux-tock-x-cat (vcat-x animal) (vcat-dir animal)) (vcat-dir animal)))]
        [(vcham? animal) (make-vcham
                         (aux-tock-x-cham (vcham-x animal) (vcham-dir animal))
                         (aux-tock-hap (vcham-hap animal))
                         (aux-tock-dir (vcham-x animal) (aux-tock-x-cham (vcham-x animal) (vcham-dir animal)) (vcham-dir animal))
                         (vcham-color animal))]))



; X-coord, Dir  -> X-coord
; aux func for tock for compute x if case Vanimal is Vcham

(define (aux-tock-x-cham x dir)
  (cond [(string=? dir ANIMAL-PATH-DIRECTION-RIGHT)
         (cond
           [(>= (+ x ANIMAL-X-INCREASE-PER-TOCK)  (-  WIDTH (/ CHAM-IMAGE-WIDTH 2))) x]
           [else (+ x ANIMAL-X-INCREASE-PER-TOCK)])]
        [(string=? dir ANIMAL-PATH-DIRECTION-LEFT)
         (cond
           [(<= (- x ANIMAL-X-INCREASE-PER-TOCK) (/ CHAM-IMAGE-WIDTH 2)) x]
           [else (- x ANIMAL-X-INCREASE-PER-TOCK)])]))



; X-coord, Dir  -> X-coord
; aux func for tock for compute x if case Vanimal is Vcat

(define (aux-tock-x-cat x dir)
  (cond [(string=? dir ANIMAL-PATH-DIRECTION-RIGHT)
         (cond
           [(>= (+ x ANIMAL-X-INCREASE-PER-TOCK)  (-  WIDTH (/ CAT-IMAGE-WIDTH 2))) x]
           [else (+ x ANIMAL-X-INCREASE-PER-TOCK)])]
        [(string=? dir ANIMAL-PATH-DIRECTION-LEFT)
         (cond
           [(<= (- x ANIMAL-X-INCREASE-PER-TOCK) (/ CAT-IMAGE-WIDTH 2)) x]
           [else (- x ANIMAL-X-INCREASE-PER-TOCK)])]))
...
(define (on-pad? animal ke)
  (cond [(or (string=? ke "up") (string=? ke "down")) (aux-on-pad-hap animal ke)]
        [(or (string=? ke "right") (string=? ke "left")) (aux-on-pad-dir animal ke)]
        [(and (or (string=? ke CHAM-COLOR-RED-KEY) (string=? ke CHAM-COLOR-GREEN-KEY) (string=? ke CHAM-COLOR-BLUE-KEY)) (vcham? animal))
         (make-vcham
           (vcham-x animal)
           (vcham-hap animal)
           (vcham-dir animal)
...
;(check-expect (aux-on-pad-hap (make-zoo (make-vcat 50 50 "right") (make-vcham 50 50 "right" "red") CAT_FOCUS_FLAG) "up") (make-zoo (make-vcat 50 (* 50 MULTIPLIE_H_PET_PET) "right") (make-vcham 50 50 "right" "red") CAT_FOCUS_FLAG))
;(check-expect (aux-on-pad-hap (make-zoo (make-vcat 50 50 "right") (make-vcham 50 50 "right" "red") CHAM_FOCUS_FLAG) "up") (make-zoo (make-vcat 50 50 "right") (make-vcham 50 (* 50 MULTIPLIE_H_PET_PET) "right" "red") CHAM_FOCUS_FLAG))
;(check-expect (aux-on-pad-hap (make-zoo (make-vcat 50 50 "right") (make-vcham 50 50 "right" "red") CAT_FOCUS_FLAG) "down") (make-zoo (make-vcat 50 (+ 50 INCREASE_H_PER_FEEDING) "right") (make-vcham 50 50 "right" "red") CAT_FOCUS_FLAG))
;(check-expect (aux-on-pad-hap 900 "up") H_MAX_LIMIT)
;(check-expect (aux-on-pad-hap 999 "down") H_MAX_LIMIT)


(define (aux-on-pad-hap animal ke)
  (cond [(and (vcat? animal) (or (string=? ke "up")
                                 (string=? ke "down")
                                 (string=? ke "right")
                                 (string=? ke "left"))) (make-vcat
                                                         (vcat-x animal)
                                                         (cond [(string=? ke "up")
                                                               ...
                                                               [else (vcat-hap animal)])
                                                         (cond [(string=? ke "right") "right"]
                                                               [(string=? ke "left") "left"]
                                                               [else (vcat-dir animal)]))]
        [(and (vcham? animal) (or (string=? ke "up")
                                  (string=? ke "right")
                                  (string=? ke "left")
                                  (string=? ke CHAM-COLOR-RED-KEY)
                                  (string=? ke CHAM-COLOR-GREEN-KEY) 
                                  (string=? ke CHAM-COLOR-BLUE-KEY))) (make-vcham
                                                                       (vcham-x animal)
                                                                       (cond [(string=? "down" ke)
                                                                              (cond [(> (+ (vcat-hap vcat) H-ADDENDUM-FOR-FEEDING-DOWN-KEY) H-MAX-LIMIT) H-MAX-LIMIT]
                                                                                    [else (+ (vcat-hap vcat) H-ADDENDUM-FOR-FEEDING-DOWN-KEY)])]
                                                                             [else (vcham-hap animal)])
                                                                       (cond [(string=? ke "right") "right"]
                                                                              [(string=? ke "left") "left"]
                                                                              [else (vcat-dir animal)])
                                                                       (cond [(string=? ke CHAM-COLOR-RED-KEY) CHAM-COLOR-RED-NAME]
